function [Deriv] = Derivative(C0,KC0,RealQ,VectorPolinoms,R,Set,Norm,Noise)
% The function of calculating the gradient of the likelihood function.
...
for t = 1:KC0
    for k = 1:N
        for l = 1:N
            Ff(t,k,l) = 4*pi*beta*imag(L(k,l)*Ff(t,k,l));
            Deriv(t) = Deriv(t) + ((RealQ(k,l) + 1)/(Q(k,l) + 1) - 1)*Ff(t,k,l);
        end
    end
end
